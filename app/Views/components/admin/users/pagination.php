...
$buildUrl = static function (int $targetPage, ?int $targetPerPage = null) use ($baseUrl, $filters, $sort, $dir, $perPage): string {
    $query = [
        ...
        'per_page' => $targetPerPage ?? $perPage,
    ];
...
<section class="admin-panel posts-pagination-panel">
  <div class="posts-pagination-shell">
    <div class="posts-pagination-summary">
      <?php if ($total > 0): ?>
        Exibindo <span><?= number_format($firstItem, 0, ',', '.') ?></span> ate <span><?= number_format($lastItem, 0, ',', '.') ?></span> de <span><?= number_format($total, 0, ',', '.') ?></span> usuarios
      <?php else: ?>
        Nenhum usuario para paginar no momento.
      <?php endif; ?>
    </div>
      ...
      <div class="posts-pagination-per-page">
        ...
        <div class="posts-pagination-chip-group">
          <?php foreach ([10, 20, 50] as $option): ?>
            ...
            <a class="posts-pagination-chip<?= $active ? ' is-active' : '' ?>" href="<?= htmlspecialchars($buildUrl(1, $option), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>"><?= $option ?></a>
          <?php endforeach; ?>
        </div>
      </div>

      <nav class="posts-pagination-nav" aria-label="Paginacao dos usuarios">
        <a class="posts-pagination-link posts-pagination-link-wide<?= $total === 0 || $page <= 1 ? ' is-disabled' : '' ?>" href="<?= htmlspecialchars($buildUrl($page - 1), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">Anterior</a>
        <?php for ($current = $start; $current <= $end; $current++): ?>
          <a class="posts-pagination-link<?= $current === $page ? ' is-active' : '' ?>" href="<?= htmlspecialchars($buildUrl($current), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>"><?= $current ?></a>
        <?php endfor; ?>
        <a class="posts-pagination-link posts-pagination-link-wide<?= $total === 0 || $page >= $pages ? ' is-disabled' : '' ?>" href="<?= htmlspecialchars($buildUrl($page + 1), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">Proxima</a>
      </nav>
    </div>
  </div>
</section>